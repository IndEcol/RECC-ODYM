...
def main():
    ...
    OutputDict      = {}  # Dictionary with output variables for check and analysis.
    ...
    GHG_OtherThanUsePhase            = np.zeros((Nt,NS,NR))
    ...
    GHG_Buildings_Direct             = np.zeros((Nt,Nr,NS,NR)) # use phase only
    ...
    for mS in range(0,NS):
        for mR in range(0,NR):
            ...
            for mSS in range(0,NS):
                pC_FutureStock[mSS, Sector_pav_loc, :] = RECC_System.ParameterDict['2_S_RECC_FinalProducts_Future_passvehicles'].Values[mSS,0,Sector_pav_loc,:]
                pC_FutureStock[mSS, Sector_reb_loc, :] = RECC_System.ParameterDict['2_S_RECC_FinalProducts_Future_resbuildings'].Values[mSS,0,Sector_reb_loc,:]
            OutputDict['pC_FutureStock']   = pC_FutureStock.copy()
            ...
            SysVar_GHGEms_UsePhase                 = np.einsum('XtrB->Xt',SysVar_DirectEmissions_UsePhase_Buildings) + np.einsum('Xtrp->Xt',SysVar_DirectEmissions_UsePhase_Vehicles)
            ...
            SysVar_GHGEms_UsePhase_OtherIndir      = SysVar_IndirectGHGEms_EnergySupply_UsePhase_Buildings_Ot + SysVar_IndirectGHGEms_EnergySupply_UsePhase_Vehicles_Ot
            ...
            SysVar_GHGEms_PrimaryMaterial_m        = SysVar_DirectEmissions_PrimaryProd + SysVar_ProcessEmissions_PrimaryProd_m + SysVar_IndirectGHGEms_EnergySupply_PrimaryProd_m
            ...
            SysVar_GHGEms_OtherThanUsePhase        = SysVar_GHGEms_UsePhase_Scope2_El + SysVar_GHGEms_UsePhase_OtherIndir + SysVar_GHGEms_PrimaryMaterial + SysVar_GHGEms_MaterialCycle
            SysVar_TotalGHGEms                     = SysVar_GHGEms_UsePhase + SysVar_GHGEms_OtherThanUsePhase + SysVar_GHGEms_RecyclingCredit + SysVar_GHGEms_GWP_bio
    
            SysVar_GHGEms_Materials                = SysVar_GHGEms_PrimaryMaterial + SysVar_GHGEms_WasteMgtRemelting
            ...
            GHG_OtherThanUsePhase[:,mS,mR]              = SysVar_GHGEms_OtherThanUsePhase[0,:].copy() # all non use-phase processes
            GHG_Materials[:,mS,mR]                      = SysVar_GHGEms_Materials[0,:].copy()
            GHG_Vehicles_Direct[:,:,mS,mR]              = np.einsum('trg->tr',SysVar_DirectEmissions_UsePhase_Vehicles[0,:,:,:]).copy()
            GHG_Buildings_Direct[:,:,mS,mR]             = np.einsum('trg->tr',SysVar_DirectEmissions_UsePhase_Buildings[0,:,:,:]).copy()
            GHG_PrimaryMaterial[:,mS,mR]                = SysVar_GHGEms_PrimaryMaterial[0,:].copy()
            GHG_PrimaryMaterial_m[:,:,mS,mR]            = SysVar_GHGEms_PrimaryMaterial_m[0,:].copy()
    ...
    newrowoffset = msf.ExcelExportAdd_tAB(Sheet,GHG_OtherThanUsePhase,newrowoffset,len(ColLabels),'GHG emissions, industries and energy supply','Mt of CO2-eq / yr',ScriptConfig['RegionalScope'],'F_9_12','Cf. Cover sheet',IndexTable.Classification[IndexTable.index.get_loc('Scenario')].Items,IndexTable.Classification[IndexTable.index.get_loc('Scenario_RCP')].Items)
    # GHG emissions, detail
    newrowoffset = msf.ExcelExportAdd_tAB(Sheet,GHG_Vehicles_Direct.sum(axis =1),newrowoffset,len(ColLabels),'GHG emissions, vehicles, use phase','Mt of CO2-eq / yr',ScriptConfig['RegionalScope'],'E_7_0 (part)','Cf. Cover sheet',IndexTable.Classification[IndexTable.index.get_loc('Scenario')].Items,IndexTable.Classification[IndexTable.index.get_loc('Scenario_RCP')].Items)
    newrowoffset = msf.ExcelExportAdd_tAB(Sheet,GHG_Buildings_Direct.sum(axis =1),newrowoffset,len(ColLabels),'GHG emissions, buildings, use phase','Mt of CO2-eq / yr',ScriptConfig['RegionalScope'],'E_7_0 (part)','Cf. Cover sheet',IndexTable.Classification[IndexTable.index.get_loc('Scenario')].Items,IndexTable.Classification[IndexTable.index.get_loc('Scenario_RCP')].Items)
    ...
    newrowoffset = msf.ExcelExportAdd_tAB(Sheet,GHG_PrimaryMaterial,newrowoffset,len(ColLabels),'GHG emissions, primary material production (redundant)','Mt of CO2-eq / yr',ScriptConfig['RegionalScope'],'E_3_0','Cf. Cover sheet',IndexTable.Classification[IndexTable.index.get_loc('Scenario')].Items,IndexTable.Classification[IndexTable.index.get_loc('Scenario_RCP')].Items)
    ...
    ax1.plot(np.arange(Model_Time_Start,Model_Time_End +1), GHG_OtherThanUsePhase[:,0,0] ,    linewidth = linewidth[2], color = MyColorCycle[ColorOrder[2],:], linestyle = '--')
    ...
    ax1.plot(np.arange(Model_Time_Start,Model_Time_End +1), GHG_OtherThanUsePhase[:,0,1] ,    linewidth = linewidth[2], color = MyColorCycle[ColorOrder[1],:], linestyle = '--')
    #plt_lgd  = plt.legend(reversed(ProxyHandlesList),reversed(LegendItems_SSP_RE),shadow = False, prop={'size':9}, loc = 'lower left')# 'upper right' ,bbox_to_anchor=(1.20, 1))

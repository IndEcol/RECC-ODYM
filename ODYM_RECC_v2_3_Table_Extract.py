...
def main(RegionalScope,ThreeSectoList):
    ...
    Resultfile2  = xlrd.open_workbook(os.path.join(RECC_Paths.results_path,ThreeSectoList[0],'ODYM_RECC_ModelResults_' + UUID + '.xls'))
    ...
    Resultsheet1 = Resultfile.sheet_by_name('Cover')
    UUID         = Resultsheet1.cell_value(3,2)
    Resultfile2  = xlrd.open_workbook(os.path.join(RECC_Paths.results_path,ThreeSectoList[-1],'ODYM_RECC_ModelResults_' + UUID + '.xls'))
    Resultsheet2 = Resultfile2.sheet_by_name('Model_Results')
    # Find the index for the recycling credit and others:
    tci = 1
    while True:

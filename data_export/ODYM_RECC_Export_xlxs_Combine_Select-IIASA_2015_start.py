...
2025-07-30, ch: also report 2015 values (for data exchange with TIMES model)
"""
                            ...
                            if rf in secs[s]: # if current folder is in list for currect scenario --> extract results!
                                if rr[j] == 'Aggregate': # use indicator for aggregate region label and add to results:
                                    ...
                                    for t in range(0,46): # read and add values
                                        Res[targetpos,t] += RECC_RS.cell(idx+offs[s],t+8).value * float(cf[j]) # Value from Excel to array #2024-01-18 convert conversion factor to float
...
# Export results to xlsx #2024-01-18 change naming and order to match IIASA template
# Define column labels
for rs in [resS,rsCS]:
    rs.cell(row=1, column=1).value = 'Model'
    rs.cell(row=1, column=2).value = 'Scenario'
    rs.cell(row=1, column=3).value = 'Region'
    rs.cell(row=1, column=4).value = 'Variable'
...
for t in range(0,20): #start with year 2005, in 5-year steps
    resS.cell(row=1, column=6+t).value = 2005 + 5*t
    
# carrying cumulative reporting along - not necessary for IIASA reporting
...
for m in range(0,nor*nos*noi):
     ...
     for rs in [resS,rsCS]:
         ...
         rs.cell(row=m+2, column=3).value = Ar[r]
...
'''
# 2025-07-30, ch: also fill in data for year 2015
for m in range(0,nor*nos*noi):
    for n in range(0,10):
        resS.cell(row=2+m, column=8+n).value = Res[m,5*n] #start in column 9 (no results for 2005, and 2010, but report 2015); only results for every fifth year, starting with year 2015
    for n in range(0,6):
        rsCS.cell(row=2+m, column=7+n).value = ResC[m,n]
